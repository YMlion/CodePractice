...
public class InternetAddress extends Address implements Cloneable {
    ...
    public static String toString(Address[] addresses, int used) {
        ...
        for (int i = 0; i < addresses.length; i++) {
            ...
            if (used + len > 76) { // overflows ...
                ...
                if (curlen > 0 && sb.charAt(curlen - 1) == ' ') sb.setLength(curlen - 1);
    ...
    static InternetAddress _getLocalAddress(Session session)
        ...
        if (session == null) {
            ...
            if (address == null) {
                ...
                if (host == null || host.length() == 0) host = getLocalHostName();
            }
        }
    ...
    private static String getLocalHostName() throws UnknownHostException {
        ...
        if (me != null) {
            ...
            if (useCanonicalHostName) host = me.getCanonicalHostName();
            if (host == null) host = me.getHostName();
            // if we can't get our name, use local address literal
            if (host == null) host = me.getHostAddress();
     ...
     */
    @SuppressWarnings("fallthrough") private static InternetAddress[] parse(String s,
            boolean strict, boolean parseHdr) throws AddressException {
        ...
        for (start = end = -1, index = 0; index < length; index++) {
            ...
            switch (c) {
                case '(': // We are parsing a Comment. Ignore everything inside.
                    ...
                    if (nesting > 0) {
                        if (!ignoreErrors) throw new AddressException("Missing ')'", s, index);
                    ...
                    if (start_personal == -1) start_personal = pindex + 1;
                ...
                case '<':
                    rfc822 = true;
                    if (route_addr) {
                        if (!ignoreErrors) throw new AddressException("Extra route-addr", s, index);
                        ...
                        if (!in_group) {
                            ...
                            if (start_personal >= 0) {
                                ma.encodedPersonal =
                                        unquote(s.substring(start_personal, end_personal).
                                                trim());
                            }
                            v.add(ma);
                    ...
                    if (index >= length) {
                        ...
                        index = rindex + 1;
                        if (start == -1) start = rindex;    // back up to include "<"
                        break;
                    }
		     ...
		     */
                ...
                case ',':    // end of an address, probably
                    ...
                    if (rfc822 && start_personal >= 0) {
                        ...
                        if (pers.trim().length() == 0) pers = null;
                    }
		 ...
		 */
                    ...
                    if (rfc822 || strict || parseHdr) {
                        if (!ignoreErrors) checkAddress(addr, route_addr, false);
                        ma = new InternetAddress();
                        ma.setAddress(addr);
                        if (pers != null) ma.encodedPersonal = pers;
                        v.add(ma);
                    } else {
                ...
                case ':':
                    rfc822 = true;
                    if (in_group) {
                        if (!ignoreErrors) throw new AddressException("Nested group", s, index);
                    }
                    if (start == -1) start = index;
                    if (parseHdr && !strict) {
		    /*
		     ...
		     */
                        if (index + 1 < length) {
                            ...
                            if (addressSpecials.indexOf(nc) >= 0) {
                                if (nc != '@') break;    // don't change in_group
			    /*
			     ...
			     */
                                for (int i = index + 2; i < length; i++) {
                                    nc = s.charAt(i);
                                    if (nc == ';') break;
                                    if (addressSpecials.indexOf(nc) >= 0) break;
                                }
                                if (nc == ';') break;    // don't change in_group
                            }
                        }
                        ...
                        if (ignoreBogusGroupName && (gname.equalsIgnoreCase("mailto")
    ...
    private static void checkAddress(String addr, boolean routeAddr, boolean validate)
        ...
        if (addr == null) throw new AddressException("Address is null");
        int len = addr.length();
        if (len == 0) throw new AddressException("Empty address", addr);
	 ...
	 */
		 ...
		 */
                if (i + 1 < len && addr.charAt(i + 1) != ' ' && addr.charAt(i + 1) != '\t') {
                    throw new AddressException(
                            "Quoted local address contains newline without whitespace", addr);
                }
            }
            if (inquote) continue;
            if (c == '@') {
                if (i == 0) throw new AddressException("Missing local name", addr);
        ...
        if (inquote) throw new AddressException("Unterminated quote", addr);
	 ...
	 */
        ...
        start = i + 1;
        if (start >= len) throw new AddressException("Missing domain", addr);

        if (addr.charAt(start) == '.') throw new AddressException("Domain starts with dot", addr);
        ...
        if (lastc == '.') throw new AddressException("Domain ends with dot", addr);
    }
