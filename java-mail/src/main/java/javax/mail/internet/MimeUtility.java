...
public class MimeUtility {
    ...
    private static final boolean ignoreUnknownEncoding =
            PropUtil.getBooleanSystemProperty("mail.mime.ignoreunknownencoding", false);
     ...
     */
    private static String encodeWord(String string, String charset, String encoding,
            boolean encodingWord) throws UnsupportedEncodingException {
    ...
    private static void doEncode(String string, boolean b64, String jcharset, int avail,
            String prefix, boolean first, boolean encodingWord, StringBuffer buf)
            throws UnsupportedEncodingException {
        ...
        if ((len > avail) && ((size = string.length()) > 1)) {
            ...
            int split = size / 2;
            if (Character.isHighSurrogate(string.charAt(split - 1))) split--;
            if (split > 0) {
                doEncode(string.substring(0, split), b64, jcharset, avail, prefix, first,
                        encodingWord, buf);
            }
            doEncode(string.substring(split, size), b64, jcharset, avail, prefix, false,
                    encodingWord, buf);
        } else {
    ...
    public static String fold(int used, String s) {
        ...
        if (used + s.length() <= 76) return makesafe(s);
    ...
    static {
        ...
        try {
            ...
            InputStream is = javax.mail.internet.MimeUtility.class.getResourceAsStream(
