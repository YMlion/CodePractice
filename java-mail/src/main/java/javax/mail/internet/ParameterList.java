...
public class ParameterList {
    ...
    private static final boolean decodeParametersStrict =
            PropUtil.getBooleanSystemProperty("mail.mime.decodeparameters.strict", false);
    ...
    private static final boolean splitLongParameters =
            PropUtil.getBooleanSystemProperty("mail.mime.splitlongparameters", true);
        ...
        for (; ; ) {
            ...
            if ((char) type == ';') {
                ...
                if (tk.getType() != HeaderTokenizer.Token.ATOM) {
                    throw new ParseException("In parameter list <"
                            + s
                            + ">"
                ...
                if (type == HeaderTokenizer.Token.ATOM && lastName != null && ((applehack && (
    ...
    private void combineMultisegmentNames(boolean keepConsistentOnFailure) throws ParseException {
		 ...
		 */
                ...
                if (segment == 0) {
                    ...
                    try {
                        if (charset != null) charset = MimeUtility.javaCharset(charset);
    ...
    public String toString(int used) {
		 ...
		 */
                if (value.length() > 60 && splitLongParameters && encodeParameters) {
                    ...
                    if (value.length() > 0) sb.addNV(name + seg, quote(value));
                } else {
                    sb.addNV(name, quote(value));
                }
            }
	 ...
	 */
        ...
        if (charset != null) charset = MimeUtility.javaCharset(charset);
        if (charset == null || charset.length() == 0) charset = MimeUtility.getDefaultJavaCharset();
