...
public class HeaderTokenizer {
    ...
    public String getRemainder() {
        if (nextPos >= string.length()) return null;
    ...
    private Token getNext(char endOfAtom, boolean keepEscapes) throws ParseException {
        ...
        while (c == '(') {
            // Parsing comment ..
            int nesting;
            for (start = ++currentPos, nesting = 1; nesting > 0 && currentPos < maxPos;
                    currentPos++) {
                c = string.charAt(currentPos);
    ...
    private Token collectString(char eos, boolean keepEscapes) throws ParseException {
        ...
        if (eos == '"') throw new ParseException("Unbalanced quoted string");
