...
public class POP3Message extends MimeMessage implements ReadableMime {
     ...
     */
    @Override public synchronized void setFlags(Flags newFlags, boolean set)
    ...
    @Override public int getSize() throws MessagingException {
	     ...
	     */
            ...
            synchronized (this) {
                if (msgSize < 0) msgSize = folder.getProtocol().list(msgnum) - hdrSize;
                return msgSize;
            }
            ...
            synchronized (this) {
                ...
                if (rawcontent == null) {
                    ...
                    if (cache != null) {
                        ...
                        rawcontent = folder.getProtocol()
                                .retr(msgnum, msgSize > 0 ? msgSize + hdrSize : 0);
                    }
                    if (rawcontent == null) {
                        expunged = true;
     ...
     * @see #contentStream
     */
    @Override protected synchronized InputStream getContentStream() throws MessagingException {
     ...
     */
    @Override public synchronized void writeTo(OutputStream os, String[] ignoreList)
            throws IOException, MessagingException {
        InputStream rawcontent = rawData.get();
        if (rawcontent == null
                && ignoreList == null
                && !((POP3Store) (folder.getStore())).cacheWriteTo) {
            if (folder.logger.isLoggable(Level.FINE)) folder.logger.fine("streaming msg " + msgnum);
            if (!folder.getProtocol().retr(msgnum, os)) {
                expunged = true;
                throw new MessageRemovedException("can't retrieve message #"
                        + msgnum
                        + " in POP3Message.writeTo");    // XXX - what else?
            }
