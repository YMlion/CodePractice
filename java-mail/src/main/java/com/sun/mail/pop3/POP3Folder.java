...
public class POP3Folder extends Folder {
     ...
     * @return NUL
     */
    @Override public char getSeparator() {
        return '\0';
    }
     ...
     * @return false
     */
    @Override public boolean create(int type) throws MessagingException {
        return false;
    }
     ...
     * @exception MessagingException    always
     */
    @Override public Folder getFolder(String name) throws MessagingException {
        throw new MessagingException("not a directory");
    }
     ...
     */
    @Override public boolean delete(boolean recurse) throws MessagingException {
     ...
     * @exception MethodNotSupportedException    always
     */
    @Override public boolean renameTo(Folder f) throws MessagingException {
    ...
    @Override public synchronized void close(boolean expunge) throws MessagingException {
	     ...
	     */
            if (store.rsetBeforeQuit && !forceClose) port.rset();
            POP3Message m;
                        ...
                        if (m.isSet(Flags.Flag.DELETED)) {
                            try {
                                ...
                                throw new MessagingException(
                                        "Exception deleting messages during close", ioex);
                            }
                        }
                    }
                }
	     ...
	     */
            for (int i = 0; i < message_cache.length; i++) {
                if ((m = message_cache[i]) != null) m.invalidate(true);
            }
    ...
    @Override public synchronized boolean isOpen() {
        ...
        try {
            if (!port.noop()) throw new IOException("NOOP failed");
        } catch (IOException ioex) {
            try {
                close(false);
    ...
    @Override public synchronized Message getMessage(int msgno) throws MessagingException {
        checkOpen();
     ...
     * @exception MethodNotSupportedException    always
     */
    @Override public void appendMessages(Message[] msgs) throws MessagingException {
    ...
    public synchronized String getUID(Message msg) throws MessagingException {
        ...
        try {
            if (!store.supportsUidl) return null;
    ...
    */
     ...
     */
    @Override protected void notifyMessageChangedListeners(int type, Message m) {
