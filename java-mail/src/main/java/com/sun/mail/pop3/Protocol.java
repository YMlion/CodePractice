...
class Protocol {
     ...
     */
    Protocol(String host, int port, MailLogger logger, Properties props, String prefix,
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(
                        "connecting to host \"" + host + "\", port " + port + ", isSSL " + isSSL);
            }
        ...
        if (!disableCapa) setCapabilities(capa());

        pipelining = hasCapability("PIPELINING") || PropUtil.getBooleanProperty(props,
                prefix + ".pipelining", false);
        if (pipelining) logger.config("PIPELINING enabled");
    }
    ...
    private final synchronized boolean getBoolProp(Properties props, String prop) {
        ...
        if (logger.isLoggable(Level.CONFIG)) logger.config(prop + ": " + val);
        return val;
    }
    ...
    private void initStreams() throws IOException {
        ...
        output = new PrintWriter(
                new BufferedWriter(new OutputStreamWriter(traceOutput, "iso-8859-1")));
    }
	 ...
	 * from the user.
	 */
        if (!r.ok) throw new IOException("STAT command failed: " + r.data);
    ...
    synchronized InputStream retr(int msg, int size) throws IOException {
        ...
        if (batch) {
            ...
            r = readResponse();
            if (r.ok) r.bytes = readMultilineResponse(size + SLOP);
            batchCommandEnd();
        } else {
            cmd = "RETR " + msg;
    ...
    synchronized boolean retr(int msg, OutputStream os) throws IOException {
        ...
        if (terr != null) {
            if (terr instanceof IOException) throw (IOException) terr;
            if (terr instanceof RuntimeException) throw (RuntimeException) terr;
    ...
    synchronized boolean stls() throws IOException {
        if (socket instanceof SSLSocket) return true;    // nothing to do
