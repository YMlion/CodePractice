...
public class POP3Store extends Store {
    ...
    public POP3Store(Session session, URLName url, String name, boolean isSSL) {
        ...
        if (dir != null) fileCacheDir = new File(dir);
    ...
    @Override protected synchronized boolean protocolConnect(String host, int portNum, String user,
    ...
    @Override public synchronized boolean isConnected() {
        ...
        try {
            if (port == null) {
                port = getPort(null);
            } else if (!port.noop()) throw new IOException("NOOP failed");
            return true;
        } catch (IOException ioex) {
    ...
    @Override public Folder getDefaultFolder() throws MessagingException {
        checkConnected();
    ...
    @Override public Folder getFolder(URLName url) throws MessagingException {
        checkConnected();
