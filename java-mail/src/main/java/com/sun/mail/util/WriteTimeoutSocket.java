...
public class WriteTimeoutSocket extends Socket {
    ...
    public WriteTimeoutSocket(InetAddress address, int port, InetAddress localAddress,
    ...
    public WriteTimeoutSocket(String host, int port, InetAddress localAddress, int localPort,
    ...
    @Override public void connect(SocketAddress remote) throws IOException {
        socket.connect(remote, 0);
    }

    @Override public void connect(SocketAddress remote, int timeout) throws IOException {
        socket.connect(remote, timeout);
    }

    @Override public void bind(SocketAddress local) throws IOException {
        socket.bind(local);
    }
...
class TimeoutOutputStream extends OutputStream {
    ...
    @Override public synchronized void write(byte[] bs, int off, int len) throws IOException {
        if ((off < 0) || (off > bs.length) || (len < 0) || ((off + len) > bs.length) || ((off + len)
                < 0)) {
        ...
        try {
            try {
                if (timeout > 0) sf = ses.schedule(timeoutTask, timeout, TimeUnit.MILLISECONDS);
