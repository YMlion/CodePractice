...
public class CompactFormatter extends java.util.logging.Formatter {
     ...
     * </pre>
     *
     * <li>{@code com.sun.mail.util.logging.CompactFormatter.format=[%9$d][%1$tT][%10$d][%2$s]
     * %5$s%n%6$s%n}
     * <p>
    ...
    @Override public String format(final LogRecord record) {
        ...
        Object[] params = {
                formatZonedDateTime(record), formatSource(record), formatLoggerName(record),
                formatLevel(record), msg, thrown, new Alternate(msg, thrown),
                new Alternate(thrown, msg), record.getSequenceNumber(), formatThreadID(record), err,
                new Alternate(msg, err), new Alternate(err, msg), formatBackTrace(record),
     ...
     */
    @Override public String formatMessage(final LogRecord record) {
    ...
    private boolean isStaticUtility(final StackTraceElement s) {
        ...
        return (cn.endsWith("s") && !cn.endsWith("es")) || cn.contains("Util") || cn.endsWith(
                "Throwables");
    }
    ...
    private boolean isReflection(final StackTraceElement s) {
        ...
        return s.getClassName().startsWith("java.lang.reflect.") || s.getClassName()
