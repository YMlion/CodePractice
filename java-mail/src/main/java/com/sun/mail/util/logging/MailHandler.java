...
public class MailHandler extends Handler {
     ...
     * positive.
     */
    @SuppressWarnings("VolatileArrayField") private volatile Filter[] attachmentFilters;
    /**
     ...
     */
    @Override public void setLevel(final Level newLevel) {
        if (newLevel == null) {
            throw new NullPointerException();
        }
        checkAccess();
     ...
     */
    @Override public void setErrorManager(final ErrorManager em) {
        checkAccess();
        setErrorManager0(em);
    }
     ...
     * does not have <tt>LoggingPermission("control")</tt>.
     */
    @Override public void setFilter(final Filter newFilter) {
        checkAccess();
        synchronized (this) {  //Wait for writeLogRecords.
            if (newFilter != filter) {
                clearMatches(-1);
     ...
     */
    @Override public synchronized String getEncoding() {
     ...
     */
    @Override public synchronized Formatter getFormatter() {
     ...
     */
    @Override public synchronized void setFormatter(Formatter newFormatter)
     ...
     */
    @Override protected void reportError(String msg, Exception ex, int code) {
        try {
            if (msg != null) {
    ...
    final String contentTypeOf(final Formatter f) {
        ...
        if (f != null) {
            ...
            for (Class<?> k = f.getClass(); k != Formatter.class; k = k.getSuperclass()) {
                ...
                for (int idx = name.indexOf('$') + 1; (idx = name.indexOf("ml", idx)) > -1;
                        idx += 2) {
                    if (idx > 0) {
     ...
     * @since JavaMail 1.4.5
     */
    @SuppressWarnings("UseSpecificCatch") private void reportError(Message msg, Exception ex,
            int code) {
        try {
            ...
            for (int i = 0; i < expect; ++i) {
                if (this.attachmentNames[i] == null) {
                    this.attachmentNames[i] =
                            TailNameFormatter.of(toString(this.attachmentFormatters[i]));
                }
            }
        }
        return fixed;
    ...
    private void initAttachmentNames(final String p) {
        ...
        if (!isEmpty(list)) {
            ...
            if (alignAttachmentNames()) { //Any null indexes are repaired.
                reportError("Attachment names.", attachmentMismatch("Length mismatch."),
                        ErrorManager.OPEN_FAILURE);
            }
        } else {
            this.attachmentNames = emptyFormatterArray();
    ...
    private boolean isPushable(final LogRecord record) {
        ...
        if ((match == -1 && getFilter() == push) || (match >= 0
                && attachmentFilters[match] == push)) {
            return true;
        } else {
    ...
    private void verifySettings0(Session session, String verify) {
        ...
        if (!"local".equals(verify)
                && !"remote".equals(verify)
                && !"limited".equals(verify)
                && !"resolve".equals(verify)
                && !"login".equals(verify)) {
    ...
    private void reportUnexpectedSend(MimeMessage msg, String verify, Exception cause) {
        final MessagingException write =
                new MessagingException("An empty message was sent.", cause);
    ...
    private void reportNonSymmetric(final Object o, final Object found) {
        reportError("Non symmetric equals implementation.", new IllegalArgumentException(
                        o.getClass().getName() + " is not equal to " + found.getClass().getName()),
                ErrorManager.OPEN_FAILURE);
    }
    ...
    private void reportNonDiscriminating(final Object o, final Object found) {
        reportError("Non discriminating equals implementation.", new IllegalArgumentException(
    ...
    private void setMailer(final Message msg) {
        try {
            ...
            if (k == mail) {
                ...
                value = MimeUtility.fold(10,
                        mail.getName() + " using the " + value + " extension.");
            }
     ...
     */
    private boolean setRecipient(final Message msg, final String key,
    ...
    private static RuntimeException attachmentMismatch(int expected, int found) {
        return attachmentMismatch(
                "Attachments mismatched, expected " + expected + " but given " + found + '.');
    }
    ...
    private static final class DefaultAuthenticator extends Authenticator {
        ...
        @Override protected final PasswordAuthentication getPasswordAuthentication() {
    ...
    private static final class TailNameFormatter extends Formatter {
        ...
        @Override public final String getTail(Handler h) {
            return name;
        }
