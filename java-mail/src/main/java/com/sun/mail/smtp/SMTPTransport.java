...
public class SMTPTransport extends Transport {
    ...
    protected SMTPTransport(Session session, URLName urlname, String name, boolean isSSL) {
        ...
        reportSuccess =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".reportsuccess",
                        false);
        ...
        noopStrict =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".noop.strict", true);
        ...
        if (useCanonicalHostName) logger.config("use canonical host name");

        allowutf8 = PropUtil.getBooleanSessionProperty(session, "mail.mime.allowutf8", false);
        if (allowutf8) logger.config("allow UTF-8");
        ...
        Authenticator[] a = new Authenticator[] {
                new LoginAuthenticator(), new PlainAuthenticator(), new DigestMD5Authenticator(),
    ...
    public synchronized String[] getSASLMechanisms() {
        if (saslMechanisms == UNKNOWN_SA) {
            ...
            if (s != null && s.length() > 0) {
                if (logger.isLoggable(Level.FINE)) logger.fine("SASL mechanisms allowed: " + s);
        ...
        if (saslMechanisms == null) return null;
    ...
    public synchronized void setSASLMechanisms(String[] mechanisms) {
        if (mechanisms != null) mechanisms = mechanisms.clone();
     ...
     */
    @Override protected synchronized boolean protocolConnect(String host, int port, String user,
            String password) throws MessagingException {
        // setting mail.smtp.auth to true enables attempts to use AUTH
        boolean useAuth =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".auth", false);
	 ...
	 */
        ...
        boolean useEhlo =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".ehlo", true);
        if (logger.isLoggable(Level.FINE)) {
	 ...
	 */
        ...
        try {
            ...
            if ((useAuth || (user != null && password != null)) && (supportsExtension("AUTH")
    ...
    private abstract class Authenticator {
		 ...
		 */
                ...
                if (resp == 334) doAuth(host, authzid, user, passwd);
    ...
    private class LoginAuthenticator extends Authenticator {
        ...
        @Override void doAuth(String host, String authzid, String user, String passwd)
                throws MessagingException, IOException {
            // send username
    ...
    private class PlainAuthenticator extends Authenticator {
        ...
        @Override String getInitialResponse(String host, String authzid, String user, String passwd)
                throws MessagingException, IOException {
            // return "authzid<NUL>user<NUL>passwd"
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            OutputStream b64os = new BASE64EncoderStream(bos, Integer.MAX_VALUE);
            if (authzid != null) b64os.write(authzid.getBytes(StandardCharsets.UTF_8));
            b64os.write(0);
    ...
    private class DigestMD5Authenticator extends Authenticator {
        ...
        private synchronized DigestMD5 getMD5() {
            if (md5support == null) md5support = new DigestMD5(logger);
            return md5support;
        }
    ...
    private class NtlmAuthenticator extends Authenticator {
        ...
        @Override String getInitialResponse(String host, String authzid, String user, String passwd)
            ...
            flags = PropUtil.getIntProperty(session.getProperties(),
        ...
        if (saslAuthenticator == null) {
            try {
                ...
                Constructor<?> c = sac.getConstructor(new Class<?>[] {
                        SMTPTransport.class, String.class, Properties.class, MailLogger.class,
                        String.class
                });
                saslAuthenticator = (SaslAuthenticator) c.newInstance(new Object[] {
                        this, name, session.getProperties(), logger, serviceHost
                });
            } catch (Exception ex) {
     ...
     * @throws MessagingException      if the connection is dead or not in the connected state or if
    ...
    public synchronized void sendMessage(Message message, Address[] addresses)
        ...
        sendMessage(message, addresses, null);
    }
     ...
     * @see javax.mail.event.TransportEvent
     */
    @Override public synchronized void sendMessage(Message message, Address[] addresses, SMTPOutputStream.WritingListener writingListener)
            throws MessagingException, SendFailedException {
        ...
        if (addresses.length == 0) throw new SendFailedException("No recipient addresses");
        ...
        if (!use8bit) {
            use8bit = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".allow8bitmime",
                    false);
        }
        if (logger.isLoggable(Level.FINE)) logger.fine("use8bit " + use8bit);
		 ...
		 */
                ...
                this.message.writeTo(bdat(writingListener), ignoreList);
                finishBdat();
            } else {
                //this.message.writeTo(data(), ignoreList);
                this.message.writeTo(data(writingListener), ignoreList);
                finishData();
            }
            if (sendPartiallyFailed) {
                ...
                logger.fine(
                        "Sending partially failed " + "because of invalid destination addresses");
                notifyTransportListeners(TransportEvent.MESSAGE_PARTIALLY_DELIVERED, validSentAddr,
                        validUnsentAddr, invalidAddr, this.message);

                throw new SMTPSendFailedException(".", lastReturnCode, lastServerResponse,
                        exception, validSentAddr, validUnsentAddr, invalidAddr);
            }
            logger.fine("message successfully delivered to mail server");
            notifyTransportListeners(TransportEvent.MESSAGE_DELIVERED, validSentAddr,
                    validUnsentAddr, invalidAddr, this.message);
        } catch (MessagingException mex) {
            ...
            addressesFailed();
            notifyTransportListeners(TransportEvent.MESSAGE_NOT_DELIVERED, validSentAddr,
                    validUnsentAddr, invalidAddr, this.message);
    ...
    private void addressesFailed() {
        if (validSentAddr != null) {
            if (validUnsentAddr != null) {
                ...
                System.arraycopy(validUnsentAddr, 0, newa, validSentAddr.length,
     ...
     * @since JavaMail 1.4.2
     */
    @Override protected void notifyTransportListeners(int type, Address[] validSent,
    ...
    private boolean is8Bit(InputStream is) {
        ...
        if (need8bit) logger.fine("found an 8bit part");
        return need8bit;
    }
    ...
    protected void mailFrom() throws MessagingException {
        ...
        if (allowutf8 && supportsExtension("SMTPUTF8")) cmd += " SMTPUTF8";
        ...
        try {
            ...
            switch (retCode) {
                ...
                case 501:
                    ...
                    try {
                        ex.setNextException(
                                new SMTPSenderFailedException(new InternetAddress(from), cmd,
    ...
    protected void rcptTo() throws MessagingException {
        ...
        if (!sendPartial) {
            sendPartial =
                    PropUtil.getBooleanSessionProperty(session, "mail." + name + ".sendpartial",
                            false);
        }
        if (sendPartial) logger.fine("sendPartial set");
        ...
        } else if (reportSuccess || (sendPartial && (invalid.size() > 0
                || validUnsent.size() > 0))) {
        ...
        if (sendFailed) {
            ...
            try {
                if (serverSocket != null) issueCommand("RSET", -1);
            ...
            throw new SendFailedException("Invalid Addresses", mex, validSentAddr, validUnsentAddr,
                    invalidAddr);
        }
    }
     ...
     * @since JavaMail 1.4.1
     */
    protected OutputStream data(SMTPOutputStream.WritingListener writingListener) throws MessagingException {
        assert Thread.holdsLock(this);
        issueSendCommand("DATA", 354);
        dataStream = new SMTPOutputStream(serverOutput);
        dataStream.setListener(writingListener);
        return dataStream;
    }
     ...
     */
    protected OutputStream bdat(SMTPOutputStream.WritingListener writingListener) throws MessagingException {
        assert Thread.holdsLock(this);
    ...
    protected void startTLS() throws MessagingException {
        ...
        try {
            serverSocket = SocketFetcher.startTLS(serverSocket, host, session.getProperties(),
                    "mail." + name);
            initStreams();
        } catch (IOException ioex) {
            closeConnection();
    ...
    private void openServer(String host, int port) throws MessagingException {
        ...
        try {
            ...
            throw new MessagingException(
                    "Could not connect to SMTP host: " + host + ", port: " + port, ioe);
        }
    }
    ...
    private void openServer() throws MessagingException {
        ...
        try {
            ...
            if ((r = readServerResponse()) != 220) {
                ...
                throw new MessagingException("Got bad greeting from SMTP host: "
                        + host
                        + ", port: "
                        + port
                        + ", response: "
            ...
            throw new MessagingException(
                    "Could not start protocol to SMTP host: " + host + ", port: " + port, ioe);
        }
    }
    ...
    public synchronized void issueCommand(String cmd, int expect) throws MessagingException {
        ...
        int resp = readServerResponse();
        if (expect != -1 && resp != expect) throw new MessagingException(lastServerResponse);
    }
    ...
    private void issueSendCommand(String cmd, int expect) throws MessagingException {
        ...
        if ((ret = readServerResponse()) != expect) {
            ...
            lastServerResponse = _lsr;    // restore, for get
            lastReturnCode = _lrc;
            throw new SMTPSendFailedException(cmd, ret, lastServerResponse, exception,
    ...
    protected int readServerResponse() throws MessagingException {
        ...
        if (returnCode == -1) logger.log(Level.FINE, "bad server response: {0}", serverResponse);
        ...
        private void bdat(byte[] b, int off, int len, boolean last) throws IOException {
            if (len > 0 || last) {
                try {
                    ...
                    if (ret != 250) throw new IOException(lastServerResponse);
                } catch (MessagingException mex) {
