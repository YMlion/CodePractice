...
public class MessageCache {
    ...
    MessageCache(IMAPFolder folder, IMAPStore store, int size) {
        ...
        if (logger.isLoggable(Level.CONFIG)) logger.config("create cache of size " + size);
    ...
    MessageCache(int size, boolean debug) {
        this.folder = null;
        logger =
                new MailLogger(this.getClass(), "messagecache", "DEBUG IMAP MC", debug, System.out);
        if (logger.isLoggable(Level.CONFIG)) logger.config("create DEBUG cache of size " + size);
        ensureCapacity(size, 1);
    }
    ...
    public IMAPMessage getMessage(int msgnum) {
        ...
        if (msg == null) {
            if (logger.isLoggable(Level.FINE)) logger.fine("create message number " + msgnum);
    ...
    public IMAPMessage getMessageBySeqnum(int seqnum) {
        ...
        if (msgnum < 0) {        // XXX - < 1 ?
            if (logger.isLoggable(Level.FINE)) logger.fine("no message seqnum " + seqnum);
            return null;
        } else {
    ...
    public void expungeMessage(int seqnum) {
        int msgnum = msgnumOf(seqnum);
        if (msgnum < 0) {
            if (logger.isLoggable(Level.FINE)) logger.fine("expunge no seqnum " + seqnum);
            return;        // XXX - should never happen
        }
        IMAPMessage msg = messages[msgnum - 1];
        if (msg != null) {
            if (logger.isLoggable(Level.FINE)) logger.fine("expunge existing " + msgnum);
            msg.setExpunged(true);
        }
	 ...
	 */
        ...
        while (oldnum <= size) {
            ...
            if (seqnumOf(oldnum) <= 0) {
                ...
                if (newnum != oldnum) {
                    ...
                    if (messages[newnum - 1] != null) messages[newnum - 1].setMessageNumber(newnum);
                }
                newnum++;
            }
            oldnum++;
        ...
        if (logger.isLoggable(Level.FINE)) logger.fine("return " + rmsgs.length);
        mlist.toArray(rmsgs);
        return rmsgs;
    }
	 ...
	 */
		 ...
		 */
                ...
                if (newnum != oldnum) {
                    ...
                    messages[newnum - 1] = messages[oldnum - 1];
                    if (messages[newnum - 1] != null) messages[newnum - 1].setMessageNumber(newnum);
                    if (seqnums != null) seqnums[newnum - 1] = seqnums[oldnum - 1];
                }
                if (seqnums != null && seqnums[newnum - 1] != newnum) keepSeqnums = true;
                newnum++;
            }
            oldnum++;
        }

        if (!keepSeqnums) seqnums = null;
        shrink(newnum, oldnum);
    ...
    private void shrink(int newend, int oldend) {
        size = newend - 1;
        if (logger.isLoggable(Level.FINE)) logger.fine("size now " + size);
        if (size == 0) {    // no messages left
            ...
            if (logger.isLoggable(Level.FINE)) logger.fine("clean " + newend + " to " + oldend);
            // clear out unused entries in array
            for (int msgnum = newend; msgnum < oldend; msgnum++) {
                messages[msgnum - 1] = null;
                if (seqnums != null) seqnums[msgnum - 1] = 0;
            }
        }
    }
    ...
    public void addMessages(int count, int newSeqNum) {
        if (logger.isLoggable(Level.FINE)) logger.fine("add " + count + " messages");
    ...
    private void ensureCapacity(int newsize, int newSeqNum) {
        if (messages == null) {
            ...
            if (logger.isLoggable(Level.FINE)) logger.fine("expand capacity to " + newsize);
            ...
            if (seqnums != null) {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine(
                            "message " + newsize + " has sequence number " + seqnums[newsize - 1]);
                }
            }
        } else if (newsize < size) {        // shrinking?
            // this should never happen
            if (logger.isLoggable(Level.FINE)) logger.fine("shrink capacity to " + newsize);
            for (int msgnum = newsize + 1; msgnum <= size; msgnum++) {
                messages[msgnum - 1] = null;
                if (seqnums != null) seqnums[msgnum - 1] = -1;
            }
        }
        size = newsize;
    }
    ...
    private int msgnumOf(int seqnum) {
        if (seqnums == null) return seqnum;
        if (seqnum < 1) {        // should never happen
            if (logger.isLoggable(Level.FINE)) logger.fine("bad seqnum " + seqnum);
            return -1;
        }
        for (int msgnum = seqnum; msgnum <= size; msgnum++) {
            if (seqnums[msgnum - 1] == seqnum) return msgnum;
            if (seqnums[msgnum - 1] > seqnum) break;        // message doesn't exist
        }
        return -1;
    }
}
