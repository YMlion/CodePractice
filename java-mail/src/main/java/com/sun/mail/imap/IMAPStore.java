...
public class IMAPStore extends Store implements QuotaAwareStore, ResponseHandler {
    ...
    static class ConnectionPool {
        ...
        ConnectionPool(String name, MailLogger plogger, Session session) {
            ...
            int size =
                    PropUtil.getIntSessionProperty(session, "mail." + name + ".connectionpoolsize",
                            -1);
            if (size > 0) {
                poolSize = size;
            ...
            int serverTimeout =
                    PropUtil.getIntSessionProperty(session, "mail." + name + ".servertimeout", -1);
            if (serverTimeout > 0) {
            ...
            // check if the default server-side timeout value is overridden
            int pruning =
                    PropUtil.getIntSessionProperty(session, "mail." + name + ".pruninginterval",
                            -1);
            if (pruning > 0) {
            ...
            if (separateStoreConnection) logger.config("dedicate a store connection");
        }
    }
    ...
    private ResponseHandler nonStoreResponseHandler = new ResponseHandler() {
        @Override public void handleResponse(Response r) {
            ...
            if (r.isBYE()) logger.fine("IMAPStore non-store connection dead");
        }
    };
    ...
    protected IMAPStore(Session session, URLName url, String name, boolean isSSL) {
        ...
        debugusername =
                PropUtil.getBooleanSessionProperty(session, "mail.debug.auth.username", true);
        debugpassword =
                PropUtil.getBooleanSessionProperty(session, "mail.debug.auth.password", false);
        logger = new MailLogger(this.getClass(), "DEBUG " + name.toUpperCase(Locale.ENGLISH),
                session);

        boolean partialFetch =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".partialfetch", true);
        if (!partialFetch) {
            blksize = -1;
            logger.config("mail.imap.partialfetch: false");
        } else {
            blksize = PropUtil.getIntSessionProperty(session, "mail." + name + ".fetchsize",
                    1024 * 16);
            if (logger.isLoggable(Level.CONFIG)) logger.config("mail.imap.fetchsize: " + blksize);
        }
        ...
        statusCacheTimeout =
                PropUtil.getIntSessionProperty(session, "mail." + name + ".statuscachetimeout",
                        1000);
        if (logger.isLoggable(Level.CONFIG)) {
        ...
        appendBufferSize =
                PropUtil.getIntSessionProperty(session, "mail." + name + ".appendbuffersize", -1);
        if (logger.isLoggable(Level.CONFIG)) {
        ...
        if (logger.isLoggable(Level.CONFIG)) logger.config("mail.imap.minidletime: " + minIdleTime);
        ...
        enableStartTLS =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".starttls.enable",
                        false);
        if (enableStartTLS) logger.config("enable STARTTLS");

        // check if STARTTLS is required
        requireStartTLS =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".starttls.required",
                        false);
        if (requireStartTLS) logger.config("require STARTTLS");

        // check if SASL is enabled
        enableSASL =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".sasl.enable", false);
        if (enableSASL) logger.config("enable SASL");

        // check if SASL mechanisms are specified
        if (enableSASL) {
            s = session.getProperty("mail." + name + ".sasl.mechanisms");
            if (s != null && s.length() > 0) {
                if (logger.isLoggable(Level.CONFIG)) logger.config("SASL mechanisms allowed: " + s);
                List<String> v = new ArrayList<>(5);
                StringTokenizer st = new StringTokenizer(s, " ,");
                while (st.hasMoreTokens()) {
                    String m = st.nextToken();
                    if (m.length() > 0) v.add(m);
                }
        ...
        if (forcePasswordRefresh) logger.config("enable forcePasswordRefresh");
        ...
        if (enableResponseEvents) logger.config("enable IMAP response events");

        // check if enableresponseevents is enabled
        enableImapEvents =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".enableimapevents",
                        false);
        if (enableImapEvents) logger.config("enable IMAP IDLE events");

        // check if message cache debugging set
        messageCacheDebug =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".messagecache.debug",
                        false);

        guid = session.getProperty("mail." + name + ".yahoo.guid");
        if (guid != null) logger.log(Level.CONFIG, "mail.imap.yahoo.guid: {0}", guid);
        ...
        if (closeFoldersOnStoreFailure) logger.config("closeFoldersOnStoreFailure");

        // check if COMPRESS is enabled
        enableCompress =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".compress.enable",
                        false);
        if (enableCompress) logger.config("enable COMPRESS");

        // check if finalizeCleanClose is enabled
        finalizeCleanClose =
                PropUtil.getBooleanSessionProperty(session, "mail." + name + ".finalizecleanclose",
                        false);
        if (finalizeCleanClose) logger.config("close connection cleanly in finalize");
        ...
        if (s != null) {
            ...
            try {
                ...
                Class<?>[] c = {
                        String.class, char.class, IMAPStore.class, Boolean.class
                };
     ...
     */
    @Override protected synchronized boolean protocolConnect(String host, int pport, String user,
        ...
        try {
            ...
            if (poolEmpty) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("trying to connect to host \""
                            + host
                            + "\", port "
                            + port
                            + ", isSSL "
	 ...
	 */
        ...
        if (!p.isAuthenticated()) authenticate(p, authzid, u, pw);
	 ...
	 */
        ...
        if (p.hasCapability("UTF8=ACCEPT") || p.hasCapability("UTF8=ONLY")) p.enable("UTF8=ACCEPT");
    }
    ...
    private void authenticate(IMAPProtocol p, String authzid, String user, String password)
        ...
        if (mechs == null) mechs = defaultAuthenticationMechanisms;
	 ...
	 */
	     ...
	     */
            if (mechs == defaultAuthenticationMechanisms) {
                String dprop =
                        "mail." + name + ".auth." + m.toLowerCase(Locale.ENGLISH) + ".disable";
                boolean disabled =
                        PropUtil.getBooleanSessionProperty(session, dprop, m.equals("XOAUTH2"));
                if (disabled) {
                    if (logger.isLoggable(Level.FINE)) {
            ...
            if (!(p.hasCapability("AUTH=" + m) || (m.equals("LOGIN") && p.hasCapability(
                    "AUTH-LOGIN")))) {
    ...
    IMAPProtocol getProtocol(IMAPFolder folder) throws MessagingException {
        ...
        while (p == null) {
            ...
            synchronized (pool) {
                ...
                // create a new one.
                if (pool.authenticatedConnections.isEmpty() || (pool.authenticatedConnections.size()
                        == 1 && (pool.separateStoreConnection || pool.storeConnectionInUse))) {

                    logger.fine("no connections in the pool, creating a new one");
                    try {
                        if (forcePasswordRefresh) refreshPassword();
    ...
    private void refreshPassword() {
        ...
        PasswordAuthentication pa =
                session.requestPasswordAuthentication(addr, port, name, null, user);
        if (pa != null) {
            user = pa.getUserName();
    ...
    boolean allowReadOnlySelect() {
        return PropUtil.getBooleanSessionProperty(session, "mail." + name + ".allowreadonlyselect",
                false);
    }
    ...
    void releaseFolderStoreProtocol(IMAPProtocol protocol) {
        if (protocol == null) return;        // should never happen
        protocol.removeResponseHandler(nonStoreResponseHandler);
        protocol.addResponseHandler(this);
        synchronized (pool) {
            pool.storeConnectionInUse = false;
    ...
    private void timeoutConnections() {
        ...
        synchronized (pool) {
            ...
            if (System.currentTimeMillis() - pool.lastTimePruned > pool.pruningInterval
                ...
                if (pool.logger.isLoggable(Level.FINE)) {
                    pool.logger.fine(
                            "checking for connections to prune: " + (System.currentTimeMillis()
                                    - pool.lastTimePruned));
                ...
                for (int index = pool.authenticatedConnections.size() - 1; index > 0; index--) {
                    p = pool.authenticatedConnections.
                            elementAt(index);
                    if (pool.logger.isLoggable(Level.FINE)) {
                        pool.logger.fine("protocol last used: " + (System.currentTimeMillis()
                                - p.getTimestamp()));
                    }
                    if (System.currentTimeMillis() - p.getTimestamp()
                            > pool.clientTimeoutInterval) {

                        pool.logger.fine("authenticated connection timed out, "
	 ...
	 */
        ...
        if (logger.isLoggable(Level.FINE)) logger.fine("IMAPStore cleanup, force " + force);
     ...
     */
    @Override public synchronized Folder getDefaultFolder() throws MessagingException {
        checkConnected();
     ...
     * Get named folder. Returns a new, closed IMAPFolder.
     */
    @Override public synchronized Folder getFolder(URLName url) throws MessagingException {
        checkConnected();
    ...
    protected IMAPFolder newIMAPFolder(String fullName, char separator, Boolean isNamespace) {
        ...
        if (f == null) f = new IMAPFolder(fullName, separator, this, isNamespace);
        return f;
    }
    ...
    protected IMAPFolder newIMAPFolder(ListInfo li) {
        ...
        if (f == null) f = new IMAPFolder(li, this);
        return f;
    }
     ...
     */
    @Override public Folder[] getPersonalNamespaces() throws MessagingException {
     ...
     */
    @Override public Folder[] getUserNamespaces(String user) throws MessagingException {
        Namespaces ns = getNamespaces();
     ...
     */
    @Override public Folder[] getSharedNamespaces() throws MessagingException {
        Namespaces ns = getNamespaces();
     ...
     */
    @Override public synchronized Quota[] getQuota(String root) throws MessagingException {
        checkConnected();
        Quota[] qa = null;
     ...
     */
    @Override public synchronized void setQuota(Quota quota) throws MessagingException {
        checkConnected();
        IMAPProtocol p = null;
        try {
            p = getStoreProtocol();
    ...
    @Override public void handleResponse(Response r) {
        ...
        if (r.isBYE()) {
            ...
            synchronized (connectionFailedLock) {
                connectionFailed = true;
                if (r.isSynthetic()) forceClose = true;
            }
            return;
        }
    }
    ...
    void handleResponseCode(Response r) {
        if (enableResponseEvents) notifyStoreListeners(IMAPStore.RESPONSE, r.toString());
