...
public class IMAPBodyPart extends MimeBodyPart implements ReadableMime {
     ...
     */
    @Override protected void updateHeaders() {
        return;
    }
    ...
    private InputStream getHeaderStream() throws MessagingException {
        if (!message.isREV1()) loadHeaders();    // will be needed below
    ...
    @Override public synchronized DataHandler getDataHandler() throws MessagingException {
        if (dh == null) {
            if (bs.isMulti()) {
                dh = new DataHandler(
                        new IMAPMultipartDataSource(this, bs.bodies, sectionId, message));
            } else if (bs.isNested() && message.isREV1() && bs.envelope != null) {
                dh = new DataHandler(
                        new IMAPNestedMessage(message, bs.bodies[0], bs.envelope, sectionId), type);
            }
        }
