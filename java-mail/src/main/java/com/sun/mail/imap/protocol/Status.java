...
public class Status {
    ...
    public Status(Response r) throws ParsingException {
        ...
        if (!r.supportsUtf8()) mbox = BASE64MailboxDecoder.decode(mbox);
        ...
        do {
            ...
            if (attr == null) throw new ParsingException("parse error in STATUS");
    ...
    public long getItem(String item) {
        ...
        if (items != null && (v = items.get(item)) != null) {
            ...
            ret = unseen;
        } else if (item.equals("HIGHESTMODSEQ")) ret = highestmodseq;
        return ret;
    }
    ...
    public static void add(Status s1, Status s2) {
        ...
        if (s2.highestmodseq != -1) s1.highestmodseq = s2.highestmodseq;
        if (s1.items == null) {
            s1.items = s2.items;
        } else if (s2.items != null) s1.items.putAll(s2.items);
    }
}
