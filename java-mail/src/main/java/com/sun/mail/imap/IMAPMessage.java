...
public class IMAPMessage extends MimeMessage implements ReadableMime {
     ...
     */
    @Override protected void setMessageNumber(int msgnum) {
    ...
    @Override protected void setExpunged(boolean set) {
    ...
    @Override public Address[] getFrom() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getFrom();
        loadEnvelope();
	 ...
	 * by MimeMessage.
	 */
        if (a == null || a.length == 0) a = envelope.sender;
        return aaclone(a);
    }

    @Override public void setFrom(Address address) throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }

    @Override public void addFrom(Address[] addresses) throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
     ...
     */
    @Override public Address getSender() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getSender();
        loadEnvelope();
    ...
    @Override public void setSender(Address address) throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
    ...
    @Override public Address[] getRecipients(Message.RecipientType type) throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getRecipients(type);
        loadEnvelope();
    ...
    @Override public void setRecipients(Message.RecipientType type, Address[] addresses)
            throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }

    @Override public void addRecipients(Message.RecipientType type, Address[] addresses)
            throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
    ...
    @Override public Address[] getReplyTo() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getReplyTo();
        loadEnvelope();
	/*
	 ...
	 * MimeMessage.
	 */
        if (envelope.replyTo == null || envelope.replyTo.length == 0) return getFrom();
    ...
    @Override public String getSubject() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getSubject();
    ...
    @Override public void setSubject(String subject, String charset) throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
    ...
    @Override public Date getSentDate() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getSentDate();
        loadEnvelope();
     ...
     */
    @Override public Date getReceivedDate() throws MessagingException {
        checkExpunged();
        if (receivedDate == null) loadEnvelope(); // have to go to the server for this
    ...
    @Override public String[] getContentLanguage() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getContentLanguage();
        loadBODYSTRUCTURE();
        if (bs.language != null) {
    ...
    public String getInReplyTo() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getHeader("In-Reply-To", " ");
        loadEnvelope();
     ...
     * as well.
     */
    @Override public synchronized String getContentType() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getContentType();
    ...
    @Override public String getDisposition() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getDisposition();
        loadBODYSTRUCTURE();
    ...
    @Override public String getEncoding() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getEncoding();
        loadBODYSTRUCTURE();
        return bs.encoding;
    }
    ...
    @Override public String getContentID() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getContentID();
        loadBODYSTRUCTURE();
        return bs.id;
    }
    ...
    @Override public String getContentMD5() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getContentMD5();
        loadBODYSTRUCTURE();
        return bs.md5;
    }
    ...
    @Override public String getDescription() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getDescription();
    ...
    @Override public void setDescription(String description, String charset)
            throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
     ...
     * Get the Message-ID.
     */
    @Override public String getMessageID() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getMessageID();
        loadEnvelope();
    ...
    @Override public String getFileName() throws MessagingException {
        checkExpunged();
        if (bodyLoaded) return super.getFileName();
    ...
    @Override protected InputStream getContentStream() throws MessagingException {
        if (bodyLoaded) return super.getContentStream();
        InputStream is = null;
     ...
     */
    @Override public synchronized DataHandler getDataHandler() throws MessagingException {
        checkExpunged();
    ...
    @Override public void setDataHandler(DataHandler content) throws MessagingException {
        throw new IllegalWriteException("IMAPMessage is read-only");
    }
     ...
     */
    @Override public InputStream getMimeStream() throws MessagingException {
        ...
        synchronized (getMessageCacheLock()) {
            try {
                ...
                if (p.isREV1()) {
                    ...
                    RFC822DATA rd =
                            p.fetchRFC822(getSequenceNumber(), "HEADER.LINES (" + name + ")");
                    if (rd != null) is = rd.getByteArrayInputStream();
                }
            } catch (ConnectionException cex) {
                throw new FolderClosedException(folder, cex.getMessage());
     ...
     */
    @Override public synchronized Flags getFlags() throws MessagingException {
        checkExpunged();
        loadFlags();
    ...
    public static class FetchProfileCondition implements Utility.Condition {
        ...
        public FetchProfileCondition(FetchProfile fp, FetchItem[] fitems) {
            ...
            if (fp.contains(FetchProfile.Item.SIZE)) needSize = true;
            ...
            if (fp.contains(IMAPFolder.FetchProfileItem.MESSAGE)) needMessage = true;
            if (fp.contains(IMAPFolder.FetchProfileItem.INTERNALDATE)) needRDate = true;
            hdrs = fp.getHeaderNames();
            for (int i = 0; i < fitems.length; i++) {
                if (fp.contains(fitems[i].getFetchProfileItem())) need.add(fitems[i]);
            }
        }
         ...
         */
        @Override public boolean test(IMAPMessage m) {
            ...
            while (it.hasNext()) {
                ...
                if (m.items == null || m.items.get(fitem.getName()) == null) return true;
            }
    ...
    protected boolean handleFetchItem(Item item, String[] hdrs, boolean allHeaders)
        ...
        // Check for header items
            ...
            if (!isHeader) {
                ...
                if (headerStream != null) h.load(headerStream);
    ...
    protected void handleExtensionFetchItems(Map<String, Object> extensionItems) {
        if (extensionItems == null || extensionItems.isEmpty()) return;
    ...
    public synchronized Object getItem(FetchItem fitem) throws MessagingException {
        ...
        if (item == null) item = fetchItem(fitem);
        return item;
    }
