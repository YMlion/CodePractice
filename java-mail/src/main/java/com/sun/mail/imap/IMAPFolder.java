...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
    ...
    public static class FetchProfileItem extends FetchProfile.Item {
         ...
         * @deprecated
         */
        @Deprecated public static final FetchProfileItem SIZE = new FetchProfileItem("SIZE");
	 ...
	 */
        ...
        if (isNamespace != null) this.isNamespace = isNamespace.booleanValue();
    }
    ...
    protected void checkExists() throws MessagingException {
        ...
        if (!exists && !exists()) throw new FolderNotFoundException(this, fullName + " not found");
    }
	...
	*/
     ...
     */
    @Override public synchronized Folder getParent() throws MessagingException {
        char c = getSeparator();
        int index;
     ...
     */
    @Override public synchronized boolean exists() throws MessagingException {
     ...
     */
    @Override public Folder[] list(String pattern) throws MessagingException {
     ...
     */
    @Override public Folder[] listSubscribed(String pattern) throws MessagingException {
    ...
    private synchronized Folder[] doList(final String pattern, final boolean subscribed)
        ...
        if (attributes != null && !isDirectory()) return new Folder[0];
	 ...
	 */
        ...
        if (li.length > 0 && li[0].name.equals(fullName + c)) start = 1; // start from index = 1
     ...
     */
    @Override public synchronized char getSeparator() throws MessagingException {
     ...
     */
    @Override public synchronized int getType() throws MessagingException {
        if (opened) {
     ...
     */
    @Override public synchronized boolean isSubscribed() {
        ListInfo[] li = null;
        final String lname;
        if (isNamespace && separator != '\0') {
            lname = fullName + separator;
     ...
     */
    @Override public synchronized void setSubscribed(final boolean subscribe)
            throws MessagingException {
     ...
     */
    @Override public synchronized boolean create(final int type) throws MessagingException {
     ...
     */
    @Override public synchronized boolean hasNewMessages() throws MessagingException {
        synchronized (messageCacheLock) {
        ...
        if (li == null) throw new FolderNotFoundException(this, fullName + " not found");

        int i = findName(li, lname);
        if (li[i].changeState == ListInfo.CHANGED) {
            return true;
        } else if (li[i].changeState == ListInfo.UNCHANGED) return false;
     ...
     */
    @Override public synchronized Folder getFolder(String name) throws MessagingException {
     ...
     * Delete this folder.
     */
    @Override public synchronized boolean delete(boolean recurse) throws MessagingException {
     ...
     * Rename this folder.
     */
    @Override public synchronized boolean renameTo(final Folder f) throws MessagingException {
     ...
     */
    @Override public synchronized void open(int mode) throws MessagingException {
        open(mode, null);
    }
		 ...
		 */
                if (rd != null) {
                    if (rd == ResyncData.CONDSTORE) {
                        ...
                        if (!protocol.isEnabled("QRESYNC")) protocol.enable("QRESYNC");
                    }
                }
		 ...
		 */
            ...
            if (mi.mode != mode) {
                if (mode == READ_WRITE
                    ...
                    ReadOnlyFolderException ife =
                            new ReadOnlyFolderException(this, "Cannot open in desired mode");
            ...
            if (mi.responses != null) {
                ...
                for (IMAPResponse ir : mi.responses) {
                    if (ir.keyEquals("VANISHED")) {
                        ...
                        if (msg != null) {
                            openEvents.add(
                                    new MessageChangedEvent(this, MessageChangedEvent.FLAGS_CHANGED,
                                            msg));
                        }
                    }
                }
     ...
     */
    @Override public synchronized void fetch(Message[] msgs, FetchProfile fp)
            throws MessagingException {
        ...
        synchronized (messageCacheLock) {
            ...
            for (int i = 0; i < r.length; i++) {
                ...
                for (int j = 0; j < count; j++) {
                    ...
                    if (item instanceof Flags && (!fp.contains(FetchProfile.Item.FLAGS)
                            || msg == null)) {
                        // Ok, Unsolicited FLAGS update.
                        unsolicitedFlags = true;
                    } else if (msg != null) msg.handleFetchItem(item, hdrs, allHeaders);
                }
                if (msg != null) msg.handleExtensionFetchItems(f.getExtensionItems());

                // If this response contains any unsolicited FLAGS
                // add it to the unsolicited response vector
                if (unsolicitedFlags) v.add(f);
            }
     ...
     */
    @Override public synchronized void setFlags(Message[] msgs, Flags flag, boolean value)
            throws MessagingException {
        checkOpened();
        ...
        synchronized (messageCacheLock) {
            try {
                ...
                if (ms == null) throw new MessageRemovedException("Messages have been removed");
     ...
     */
    @Override public synchronized void setFlags(int start, int end, Flags flag, boolean value)
            throws MessagingException {
        checkOpened();
     ...
     */
    @Override public synchronized void setFlags(int[] msgnums, Flags flag, boolean value)
            throws MessagingException {
        checkOpened();
     ...
     * Close this folder.
     */
    @Override public synchronized void close(boolean expunge) throws MessagingException {
        close(expunge, false);
    }
	     ...
	     */
            ...
            try {
                ...
                if (force) {
                    ...
                    if (expunge && protocol != null) protocol.close();
                        ...
                        try {
                            if (protocol != null && protocol.hasCapability("UNSELECT")) {
                                ...
                                if (protocol != null) {
                                    ...
                                    if (selected && protocol != null) protocol.close();
                                }
                            }
                ...
                if (opened) cleanup(reuseProtocol);
            }
        }
    }
     ...
     */
    @Override public synchronized Flags getPermanentFlags() {
        if (permanentFlags == null) return null;
     ...
     */
    @Override public synchronized int getMessageCount() throws MessagingException {
        synchronized (messageCacheLock) {
            if (opened) {
     ...
     */
    @Override public synchronized int getNewMessageCount() throws MessagingException {
        synchronized (messageCacheLock) {
            if (opened) {
     ...
     */
    @Override public synchronized int getUnreadMessageCount() throws MessagingException {
        if (!opened) {
            checkExists();
            // If this folder is not yet open, we use STATUS to
     ...
     */
    @Override public synchronized int getDeletedMessageCount() throws MessagingException {
        if (!opened) {
            checkExists();
     ...
     */
    @Override public synchronized Message getMessage(int msgnum) throws MessagingException {
        checkOpened();
        checkRange(msgnum);
     ...
     * {@inheritDoc}
     */
    @Override public synchronized Message[] getMessages() throws MessagingException {
	/*
     ...
     */
    @Override public synchronized void appendMessages(Message[] msgs) throws MessagingException {
     ...
     */
    @Override public synchronized void copyMessages(Message[] msgs, Folder folder)
            throws MessagingException {
     ...
     * @since JavaMail 1.5.4
     */
    private synchronized AppendUID[] copymoveUIDMessages(Message[] msgs, Folder folder,
        ...
        {
            throw new MessagingException(move ? "can't moveUIDMessages to a different store"
     ...
     */
    @Override public synchronized Message[] expunge() throws MessagingException {
        return expunge(null);
    }
    ...
    public synchronized Message[] expunge(Message[] msgs) throws MessagingException {
        ...
        synchronized (messageCacheLock) {
		    ...
		    /* remove this message from the UIDTable */
                    long uid = m.getUID();
                    if (uid != -1) uidTable.remove(Long.valueOf(uid));
                }
            }
     ...
     */
    @Override public synchronized Message[] search(SearchTerm term) throws MessagingException {
        checkOpened();
        ...
        try {
            ...
            synchronized (messageCacheLock) {
                ...
                if (matches != null) matchMsgs = getMessagesBySeqNumbers(matches);
            }
            return matchMsgs;
        } catch (CommandFailedException cfx) {
            ...
            if (((IMAPStore) store).throwSearchException()) throw sex;
            return super.search(term);
        } catch (ConnectionException cex) {
            throw new FolderClosedException(this, cex.getMessage());
        } catch (ProtocolException pex) {
     ...
     * messages are found.
     */
    @Override public synchronized Message[] search(SearchTerm term, Message[] msgs)
            throws MessagingException {
        checkOpened();
     ...
     */
    @Override public synchronized void addMessageCountListener(MessageCountListener l) {
     ...
     */
    @Override public synchronized long getUIDValidity() throws MessagingException {
        ...
        if (status == null) throw new MessagingException("Cannot obtain UIDValidity");
     ...
     */
    @Override public synchronized long getUIDNext() throws MessagingException {
        if (opened) // we already have this information
        {
            return uidnext;
        }
        ...
        if (status == null) throw new MessagingException("Cannot obtain UIDNext");
     ...
     */
    @Override public synchronized Message getMessageByUID(long uid) throws MessagingException {
     ...
     */
    @Override public synchronized Message[] getMessagesByUID(long start, long end)
            throws MessagingException {
     ...
     */
    @Override public synchronized Message[] getMessagesByUID(long[] uids)
            throws MessagingException {
        checkOpened(); // insure that folder is open
     ...
     */
    @Override public synchronized long getUID(Message message) throws MessagingException {
    ...
    private Message[] createMessagesForUIDs(long[] uids) {
        ...
        for (int i = 0; i < uids.length; i++) {
            IMAPMessage m = null;
            if (uidTable != null) m = uidTable.get(Long.valueOf(uids[i]));
            if (m == null) {
    ...
    public synchronized long getHighestModSeq() throws MessagingException {
        ...
        if (status == null) throw new MessagingException("Cannot obtain HIGHESTMODSEQ");
    ...
    boolean startIdle(final IdleManager im) throws MessagingException {
        ...
        synchronized (this) {
            ...
            Boolean started =
                    (Boolean) doOptionalCommand("IDLE not supported", new ProtocolCommand() {
                        @Override public Object doCommand(IMAPProtocol p) throws ProtocolException {
    ...
    boolean handleIdle(boolean once) throws MessagingException {
        ...
        do {
			 ...
			 */
                        ...
                        if (ex instanceof InterruptedIOException && ((InterruptedIOException) ex).
                                bytesTransferred == 0) {
     ...
     * @since JavaMail 1.5.1
     */
    @SuppressWarnings("unchecked") public Map<String, String> id(
            final Map<String, String> clientParams) throws MessagingException {
        checkOpened();
     ...
     */
    @Override public void handleResponse(Response r) {
        assert Thread.holdsLock(messageCacheLock);
	 ...
	 */
        if (r.isBYE()) {
            ...
            if (r.readByte() == '[') {
                String s = r.readAtom();
                if (s.equalsIgnoreCase("HIGHESTMODSEQ")) highestmodseq = r.readLong();
            }
            r.reset();
            return;
	...
	/* Now check whether this is an IMAP specific response */
        ...
        if (ir.keyEquals("EXISTS")) { // EXISTS
            ...
            if (s == null) {    // no (EARLIER)
                ...
                for (Message m : msgs) {
                    if (m.getMessageNumber() > 0) messageCache.expungeMessage(m.getMessageNumber());
                }
                if (doExpungeNotification && hasMessageCountListener) {
            ...
            assert ir instanceof FetchResponse : "!ir instanceof FetchResponse";
            Message msg = processFetchResponse((FetchResponse) ir);
            if (msg != null) notifyMessageChangedListeners(MessageChangedEvent.FLAGS_CHANGED, msg);
    ...
    protected synchronized void throwClosedException(ConnectionException cex)
        ...
        if ((protocol != null && cex.getProtocol() == protocol) || (protocol == null
                && !reallyClosed)) {
            throw new FolderClosedException(this, cex.getMessage());
        } else {
            throw new StoreClosedException(store, cex.getMessage());
    ...
    protected IMAPProtocol getProtocol() throws ProtocolException {
        ...
        if (protocol == null) throw new ConnectionException("Connection closed");
        return protocol;
    }
    ...
    protected void keepConnectionAlive(boolean keepStoreAlive) throws ProtocolException {
        ...
        if (System.currentTimeMillis() - protocol.getTimestamp() > 1000) {
            waitIfIdle();
            if (protocol != null) protocol.noop();
        }
...
class LengthCounter extends OutputStream {
    ...
    @Override public void write(byte b[], int off, int len) {
        if ((off < 0) || (off > b.length) || (len < 0) || ((off + len) > b.length) || ((off + len)
                < 0)) {
