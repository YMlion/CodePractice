...
public class Protocol {
     ...
     */
    public Protocol(String host, int port, Properties props, String prefix, boolean isSSL,
    ...
    public synchronized Response[] command(String command, Argument args) {
        ...
        if (byeResp != null) v.add(byeResp);    // must be last
    ...
    public synchronized void startTLS(String cmd) throws IOException, ProtocolException {
        if (socket instanceof SSLSocket) return;    // nothing to do
	 ...
	 */
        ...
        traceInput = new TraceInputStream(new InflaterInputStream(socket.getInputStream(), inf),
                traceLogger);
        traceInput.setQuote(quote);
        input = new ResponseInputStream(traceInput);
        ...
        int strategy = PropUtil.getIntProperty(props, prefix + ".compress.strategy",
        ...
        try {
            traceOutput =
                    new TraceOutputStream(cons.newInstance(socket.getOutputStream(), def, true),
                            traceLogger);
        } catch (Exception ex) {
